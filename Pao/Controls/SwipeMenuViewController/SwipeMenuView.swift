...
public protocol SwipeMenuViewDataSource: class {
    ...
    func swipeMenuView(_ swipeMenuView: SwipeMenuView, showDotForTabAt index: Int) -> Bool?
    
    /// Return images to be displayed at the tab in `SwipeMenuView`.
    func swipeMenuView(_ swipeMenuView: SwipeMenuView, imageForPageAt index: Int) -> UIImage?
...
open class SwipeMenuView: UIView {
    ...
    // Was private(set) but now used by SearchViewController to set the currentIndex after the jump is completed [EG]
    ...
    public func updateDotOnTabView() {
        tabView?.updateDot()
    }
...
extension SwipeMenuView: TabViewDelegate, TabViewDataSource {
    ...
    public func tabView(_ tabView: TabView, didSelectTabAt index: Int) {
        ...
        guard let contentScrollView = contentScrollView else { return }
        guard currentIndex == 0 || currentIndex != index else { return }
    ...
    public func tabView(_ tabView: TabView, showDotForTabAt index: Int) -> Bool? {
        return dataSource?.swipeMenuView(self, showDotForTabAt: index)
    }
    
    // Added image support for the menu tab
    public func tabView(_ tabView: TabView, imageForItemAt index: Int) -> UIImage? {
