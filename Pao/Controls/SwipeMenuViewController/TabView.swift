...
public protocol TabViewDataSource: class {
    ...
    /// Return boolean to indicate dot will displayed or not at the tab in `TabView`.
    func tabView(_ tabView: TabView, showDotForTabAt index: Int) -> Bool?
    
    /// Return images to be displayed at the tab in `TabView`.
    func tabView(_ tabView: TabView, imageForItemAt index: Int) -> UIImage?
}
...
open class TabView: UIScrollView {
    ...
    func updateDot() {
        for (index, tabItemView) in itemViews.enumerated() {
            tabItemView.dotView.isHidden = !(dataSource?.tabView(self, showDotForTabAt: index) ?? false)
        }
    }
    ...
    fileprivate func setupTabItemViews(dataSource: TabViewDataSource) {
        ...
        for index in 0..<itemCount {
            ...
            var style: TabItemView.Style = .label(text: "")

            if let image = dataSource.tabView(self, imageForItemAt: index) {
                style = .imageView(image: image)
            } else if let title = dataSource.tabView(self, titleForItemAt: index) {
                style = .label(text: title)
            }

            let tabItemView = TabItemView(frame: CGRect(x: xPosition, y: 0, width: options.itemView.width, height: containerView.frame.size.height), style: style)
            ...
            case .label(let title):
                ...
                tabItemView.titleLabel.textAlignment = .center
            case .imageView(let image):
                tabItemView.imageView.image = image
                tabItemView.textColor = options.itemView.textColor
                tabItemView.selectedTextColor = options.itemView.selectedTextColor
            }
            ...
            tabItemView.dotView.frame = CGRect(x: tabItemView.frame.width-8, y: 0, width: 8, height: 8)
            tabItemView.dotView.isHidden = !(dataSource.tabView(self, showDotForTabAt: index) ?? false)
            tabItemView.addSubview(tabItemView.dotView)
